    ...
    const TableDemo = () => {
        ...
        const columns = dynamicColumns.map((col) => <Column key={col.field} field={col.field} header={col.header} body={(rowData) => (col.field === 'status' ? statusBodyTemplate(rowData) : rowData[col.field])} />);

        // const expiresOnBodyTemplate = (rowData: SubscriptionsData) => {
        ...
        const statusBodyTemplate = (rowData: SubscriptionsData) => {
            ...
            return (
                <span>
                    {icon} {isActive ? 'Active' : ''} {formattedDate}
                    <style jsx>{`
                        .icon-green {
            ...
            return (
                <div className="flex justify-content-between">
                    ...
                    <span className="p-input-icon-left">
                        ...
                        <InputText value={globalFilterValue1} onChange={onGlobalFilterChange1} placeholder="Search by Product name" />
                    </span>
                </div>
            );
        };
